...
def predict(test_file, output_filename, model_path, batch_size, num_workers, rank, device_id, assembly_fasta=None):
    """
            ...
            if assembly_dict is not None:
                for i in range(images.size(0)):
                    ...
                    for j in range(predicted_base_labels.shape[1]):
                        pileup_row = images[i, j].cpu().numpy()
...
def setup(rank, total_callers, args, all_input_files, all_devices):
    ...
    output_filepath, model_path, batch_size, num_workers, assembly_fasta = args
...
def predict_gpu(file_chunks, output_filepath, model_path, batch_size, total_callers, devices, num_workers, assembly_fasta=None):
    """
    ...
    args = (output_filepath, model_path, batch_size, num_workers, assembly_fasta)
